...
export const submitLeadToWebhook = async (leadData) => {
  ...
  if (DUMMY_MODE || !USE_PABBLY) {
    ...
    const existingLeads = JSON.parse(localStorage.getItem('gnrc_franchise_test_leads') || '[]');
    existingLeads.push(enrichedData);
    localStorage.setItem('gnrc_franchise_test_leads', JSON.stringify(existingLeads));

    console.log(`[DUMMY MODE] Lead stored. Total test leads: ${existingLeads.length}`);
    console.log('To view all test leads, run in console: JSON.parse(localStorage.getItem("gnrc_franchise_test_leads"))');
